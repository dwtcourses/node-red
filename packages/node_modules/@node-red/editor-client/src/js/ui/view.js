...
RED.view = (function() {
    ...
    function init() {
        ...
        eventLayer.on("mousemove", canvasMouseMove)
        ...
        function force() {
            ...
            function apply(quad, x0, y0, x1, y1) {
                ...
                if (data) {
                    ...
                    var xd = Math.abs(x) - (xSize + margin)
                    var yd = Math.abs(y) - (ySize + margin)
    ...
    function boundedBox() {
        ...
        function force() {
            ...
            while (++i < nodes.length) {
                ...
                if (x0 > margin || x1 < margin * -1) {
                    node.x += node.vx
                    node.vx = -node.vx
                    if (node.vx < x0) { node.x += x0 - node.vx }
                    if (node.vx > x1) { node.x += x1 - node.vx }
                }
                if (y0 > margin || y1 < margin * -1) {
                    node.y += node.vy
                    node.vy = -node.vy
    ...
    function canvasMouseMove() {
        ...
        if (mouse_mode !== 0) {
            if (!forceG) {
                if (!activeNodes) updateActiveNodes()
                for (i = 0; i < activeNodes.length; i++){
                    activeNodes.vx = 0
                    activeNodes.vy = 0
                }
                for (i = 0; i < moving_set.length; i++){
                    if (moving_set[i].dx > moving_set[i].dy) {
                        moving_set[i].n.vx = moving_set[i].dx
                        moving_set[i].n.vy = 0 //moving_set[i].dy
                    } else {
                        moving_set[i].n.vx = 0 //moving_set[i].dx
                        moving_set[i].n.vy = moving_set[i].dy
                    }
                }
                collisionForce = rectCollide().size(function (d) { return [d.w, d.h] }).strength(1)
                boxForce = boundedBox()
                    .bounds([[0, 0], [space_width, space_height]])
                    .size(function (d) { return [d.w, d.h] })
                forceG = d3.forceSimulation()
                    .stop()
                    .nodes(activeNodes)
                    .velocityDecay(0.2)
                    .alphaTarget(.5)
                ...
                // console.log("before tick>>", activeNodes.map(function(v) { return [v.x, v.y].join(",")}).join(";"))
                forceG.tick()
                // console.log("after tick>>", activeNodes.map(function(v) { return [v.x, v.y].join(",")}).join(";"))
                var node = nodeLayer.selectAll(".red-ui-flow-node-group").data(activeNodes,function(d){return d.id});
                node
                    .attr('x', function(d) { d.dirty = true; return d.x })
                    .attr('y', function(d) { d.dirty = true; return d.y })
                    // .attr('transform', function (d) { console.log("tick>>translate(" + (d.x-d.w/2) + "," + (d.y-d.h/2) + ")", d); d.dirty = true; return "translate(" + (d.x-d.w/2) + "," + (d.y-d.h/2) + ")" })
            }
            redraw();
        }
    }
    ...
    function redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            if (activeSubflow) {
                ...
                outGroup.on("mouseup",nodeMouseUp)
                ...
                outGroup.on("mousedown", function(d,i){portMouseDown(d,PORT_TYPE_INPUT,0);} )
                ...
                inGroup.append("rect").attr("class","red-ui-flow-subflow-port").attr("rx",8).attr("ry",8).attr("width",40).attr("height",40)
                    // TODO: This is exactly the same set of handlers used for regular nodes - DRY
                inGroup.on("mouseup",nodeMouseUp)
                    .on("mousedown",nodeMouseDown)
                    .on("touchstart",function(d) {
                            var obj = d3.select(this);
                            var touch0 = d3.event.touches.item(0);
                ...
                inGroup.on("mousedown", function(d,i){portMouseDown(d,PORT_TYPE_OUTPUT,i);} )
                ...
                statusGroup.append("rect").attr("class","red-ui-flow-subflow-port").attr("rx",8).attr("ry",8).attr("width",40).attr("height",40)
                    // TODO: This is exactly the same set of handlers used for regular nodes - DRY
                statusGroup.on("mouseup",nodeMouseUp)
                    .on("mousedown",nodeMouseDown)
                    .on("touchstart",function(d) {
                            var obj = d3.select(this);
                            var touch0 = d3.event.touches.item(0);
            ...
            nodeEnter.each(function(d,i) {
                    ...
                    if (d._def.button) {
                        ...
                        nodeButtonGroup.append("rect")
                            ...
                            .attr("cursor","pointer")
                        nodeButtonGroup.on("mousedown",function(d) {if (!lasso && isButtonEnabled(d)) {focusView();d3.select(this).attr("fill-opacity",0.2);d3.event.preventDefault(); d3.event.stopPropagation();}})
